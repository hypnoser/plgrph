...
var DICT = {
  ...
  r_brief_title: "Перед початком",
  r_consent_title: "Ознайомлення перед анкетуванням",
  ...
  r_go: "Почати",
  r_consent_default: "Участь добровільна. Ви можете попросити пояснення, відмовитися від окремої відповіді або припинити анкетування.",
  r_cs_pen: "Писати", r_cs_eraser: "Стерти", r_cs_clear: "Очистити", r_cs_print: "Друкувати",
  r_why_sign: "Поставте підпис унизу сторінки",
  r_why_confirm: "Підтвердіть відповіді цього блоку",
  r_why_fill_all: "Заповніть усі відповіді цього блоку.",
  r_why_sign_block: "Поставте підпис цього блоку.",
  r_why_ack: "Підтвердіть ознайомлення перед початком",
  r_why_identity: "Заповніть усі три поля вгорі",
  r_autoclose: "Вікно закриється автоматично через {n} с…",
  r_done_note: "Дякуємо. Передайте планшет поліграфологу.",
  r_done_head: "Анкету заповнено",
  r_aborted_head: "Анкетування припинено",
  r_aborted_note: "Анкетування припинено. Передайте планшет поліграфологові.",
  ...
  w_yes_rare: "повідомлена обставина, яка для цього шлюзу очікувалась рідкісною",
...
/* ------------------------------------------------------------------ */
/*  МОВА ЕКРАНА РЕСПОНДЕНТА (окремо від панелі поліграфолога)          */
/* ------------------------------------------------------------------ */
/* Панель поліграфолога (стартовий екран, монітор, звіт, налаштування)
   лишається виключно українською — DICT вище її й обслуговує без змін.
   Респондент може не володіти українською, тож для екрана заповнення
   (03_fill.js: кнопки так/ні/поясню, брифінг r_brief_1..5, підтвердження
   блоку, підписи полів ідентифікації тощо) є окремий, явно обраний
   поліграфологом перемикач — незалежно від questionnaire.meta.language
   (те поле стосується мови ТЕКСТУ ПИТАНЬ у самому JSON, не інтерфейсу).
   DICT_RESPONDENT_RU містить лише ті ключі, які реально використовує
   03_fill.js/briefing() — підмножина, встановлена статичним аналізом
   викликів R()/T() у коді (жоден із цих ключів не використовується на
   панелі поліграфолога, тому переклад тут нічого іншого не зачіпає). */
var DICT_RESPONDENT_RU = {
  r_yes: "да", r_no: "нет", r_explain: "поясню",
  r_erase: "стереть",
  r_block: "Блок {n} из {of}",
  r_expanded: "Раскрытие {id} · открыто ответом «{answer}»",
  r_explanation: "Пояснение к шлюзу {id}",
  r_sign_final: "Подпись под анкетой в целом",
  r_block_confirm: "Я подтверждаю, что на вышеуказанные вопросы мною даны правдивые ответы.",
  r_next: "Далее", r_finish: "Завершить",
  r_review: "Просмотреть предыдущие ответы",
  r_review_head: "К какому вопросу Вы хотите вернуться?",
  r_review_body: "Выберите уже пройденный блок и вопрос. Возврат будет зафиксирован.",
  r_review_cancel: "Остаться в текущем блоке",
  r_return_here: "Подтвердить блок и вернуться",
  r_left_one: "Остался вопрос {list}",
  r_left_many: "Остались вопросы: {list}",
  r_addendum: "Дополнение к блоку (по желанию)",
  r_fio: "Фамилия, имя и отчество",
  r_birth: "Дата рождения",
  r_doc: "Серия и номер документа, удостоверяющего личность",
  r_position: "Должность (при необходимости)",
  r_ack: "Я прочитал(-а) информацию и хочу начать анкетирование",
  r_brief_title: "Перед началом",
  r_consent_title: "Ознакомление перед анкетированием",
  r_brief_1: "Отвечайте стилусом, касаясь одного из квадратов под вопросом.",
  r_brief_2: "«да» — это было. «нет» — этого не было. «поясню» — хотите уточнить устно.",
  r_brief_3: "После «да» или «поясню» откроются дополнительные вопросы того же блока.",
  r_brief_4: "Если вопрос непонятен — спросите вслух, полиграфолог рядом.",
  r_brief_5: "Поставьте парафу в конце каждого блока и подпись в конце анкеты.",
  r_go: "Начать",
  r_consent_default: "Участие добровольное. Вы можете попросить разъяснение, отказаться от отдельного ответа или прекратить анкетирование.",
  r_cs_pen: "Писать", r_cs_eraser: "Стереть", r_cs_clear: "Очистить", r_cs_print: "Печать",
  r_why_sign: "Поставьте подпись внизу страницы",
  r_why_confirm: "Подтвердите ответы этого блока",
  r_why_fill_all: "Заполните все ответы этого блока.",
  r_why_sign_block: "Поставьте подпись этого блока.",
  r_why_ack: "Подтвердите ознакомление перед началом",
  r_why_identity: "Заполните все три поля вверху",
  r_autoclose: "Окно закроется автоматически через {n} с…",
  r_done_note: "Спасибо. Передайте планшет полиграфологу.",
  r_done_head: "Анкета заполнена",
  r_aborted_head: "Анкетирование прекращено",
  r_aborted_note: "Анкетирование прекращено. Передайте планшет полиграфологу."
};
/* Ключі DICT, яких DICT_RESPONDENT_RU свідомо НЕ перекладає (лишаються
   українською навіть при увімкненому тумблері respondent-мови):
   r_sign — ніде не використовується (мертвий рядок в оригінальному
   DICT). r_position раніше теж вважався мертвим при першому статичному
   аналізі викликів R()/T(), але насправді застосовується до плейсхолдера
   поля "Посада" в body.html (id="fb-position-label") через окремий
   виклик у paintStatic() — виправлено, тепер входить у
   DICT_RESPONDENT_RU нижче. Усі інші ключі DICT
   (s_end_*, m_mark_*, m_tip_*, w_*, o_up_word/o_down_word, o_applied)
   належать виключно панелі поліграфолога (05_monitor.js, 06_report.js,
   04_stat.js, 08_app.js) і в цей словник свідомо не входять. */

var RESPONDENT_LANG = "uk"; /* "uk" | "ru" — встановлюється тумблером на стартовому екрані */
function setRespondentLang(lang){ RESPONDENT_LANG = (lang === "ru") ? "ru" : "uk"; }
function getRespondentLang(){ return RESPONDENT_LANG; }

/* Масштаб тексту на екрані респондента (заповнення + згода) — окрема
   від браузерного/системного zoom функція, для респондентів з поганим
   зором. Свідомо НЕ пов'язана з жодною формулою в 04_stat.js: аналітика
   рахує латентність відносно q.text.length (кількість символів у тексті
   питання з файлу анкети) — величина, яка від розміру шрифту на екрані
   не залежить узагалі. Просто більший шрифт не позначає читання
   довшим/коротшим для системи підрахунку; респондент фізично витрачає
   стільки часу, скільки йому реально треба прочитати той самий текст,
   і саме цей реальний час і фіксується — незалежно від кегля. Три
   ступені (100/150/200%) — свідомо дискретні, не довільний повзунок:
   передбачувані кроки простіше перевірити на UI-регресії (переноси
   рядків, вихід за межі картки) і респонденту зрозуміліше перемикати
   одним дотиком, ніж підбирати довільне число повзунком. */
var TEXT_ZOOM = 100; /* 100 | 150 | 200 */
function setTextZoom(pct){
  TEXT_ZOOM = (pct === 150 || pct === 200) ? pct : 100;
  return TEXT_ZOOM;
}
function getTextZoom(){ return TEXT_ZOOM; }

/* T_R()/R_R() — те саме, що T()/R(), але для екрана респондента:
   якщо RESPONDENT_LANG === "ru" і ключ є в DICT_RESPONDENT_RU, береться
   переклад; інакше — як і раніше, з основного DICT. Панель поліграфолога
   і надалі використовує лише T()/R() напряму й ніколи не бачить цей
   вибір. */
function T_R(key, vars){
  var src = (RESPONDENT_LANG === "ru" && Object.prototype.hasOwnProperty.call(DICT_RESPONDENT_RU, key)) ? DICT_RESPONDENT_RU : DICT;
  return fill(Object.prototype.hasOwnProperty.call(src, key) ? src[key] : key, vars);
}
function R_R(key, vars){ return T_R(key, vars); }
...
function briefing(){
  var out = [];
  for (var i = 1; i <= 5; i++) out.push(R_R("r_brief_" + i));
...
var PI_VALIDATE = (function(){
  ...
  function run(data){
    ...
    if (!isObj(data.meta)) err("meta", "розділ відсутній або не є об'єктом");
      ...
      if (!("language" in data.meta)) wrn("meta.language", "мову не вказано — це поле стосується лише мови ТЕКСТУ ПИТАНЬ у файлі; мова кнопок і підказок на екрані респондента обирається окремим тумблером у двигуні, незалежно від цього поля");
