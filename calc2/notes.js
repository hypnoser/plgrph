// notes.js — NOTES_API v7.1
// Вкладка «Нотатки»: Markdown-редактор + зображення в IndexedDB
// Нотатки (текст) → localStorage (через collectGlobalState/APP_API)
// Зображення → IndexedDB (ключ: 'img_' + ім'я файлу + '_' + timestamp)
...
window.NOTES_API = (function () {
  ...
  function dbSaveImage(imgObj, callback) {
    openDB(function (err, database) {
      ...
      tx.onerror = function (e) { callback('Помилка запису: ' + e.target.errorCode); };
    });
  }
  ...
  function dbGetImage(id, callback) {
    openDB(function (err, database) {
      ...
      req.onerror = function (e) { callback('Помилка читання: ' + e.target.errorCode); };
    });
  }
  ...
  function dbGetAllImages(ids, callback) {
    // Завантажує масив зображень за масивом id
  ...
  var state = {
    text: '',
    imagesMeta: []   // [{id, name, size, type, added}]
  };

  var editMode = true; // true = редагування, false = перегляд
  var elements = {};  // DOM-кеш
  ...
  // ── Рендер галереї мініатюр ────────────────────────────────
  function renderThumbnails() {
    var grid = elements.thumbGrid;
    if (!grid) return;
    grid.innerHTML = '';

    if (state.imagesMeta.length === 0) {
      grid.innerHTML = '<div class="notes-no-images">Зображень ще немає. Перетягніть сюди або натисніть «＋ Додати зображення».</div>';
      return;
    }
    ...
    dbGetAllImages(ids, function (err, imgs) {
      ...
      state.imagesMeta.forEach(function (meta) {
        ...
        thumb.setAttribute('data-id', meta.id);
        ...
        if (imgObj && imgObj.dataUrl) {
          ...
          placeholder.className = 'notes-thumb-placeholder';
        ...
        delBtn.addEventListener('click', function (e) {
          e.stopPropagation();
          if (confirm('Видалити зображення «' + meta.name + '»?')) {
  ...
  function processImageFiles(files) {
    ...
    Array.from(files).forEach(function (file) {
      if (!file.type.startsWith('image/')) {
        showToast('«' + file.name + '» — не є зображенням, пропущено.', 'warn');
        return;
      }
      if (file.size > MAX_FILE_SIZE) {
        showToast('«' + file.name + '» перевищує 5 МБ, пропущено.', 'warn');
        return;
      }
      if (state.imagesMeta.length >= MAX_FILES) {
        showToast('Досягнуто ліміту ' + MAX_FILES + ' зображень.', 'warn');
        return;
      }
      ...
      reader.onload = function (evt) {
        ...
        var imgObj = { id: id, dataUrl: evt.target.result };
        dbSaveImage(imgObj, function (err) {
          if (err) { showToast('Помилка збереження: ' + err, 'error'); return; }
  ...
  function setMode(mode) {
    ...
    if (editMode) {
      ...
      elements.preview.innerHTML = renderMarkdown(state.text) || '<p style="color:#aaa; font-style:italic;">Нотаток ще немає. Перейдіть в режим редагування щоб додати текст.</p>';
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      /* Панель перемикача Edit/Preview */
      ...
      /* Textarea */
      '.notes-textarea{width:100%;min-height:280px;padding:14px;font-size:14px;font-family:"SFMono-Regular",Consolas,"Liberation Mono",Menlo,monospace;line-height:1.6;border:1px solid #ccc;border-radius:6px;resize:vertical;outline:none;box-sizing:border-box;background:#fafafa;color:#222;transition:border-color .2s;}',
      ...
      /* Print */
      '@media print{',
      '.notes-mode-bar,.notes-images-section,.notes-drop-zone,.notes-add-btn,.notes-thumb-del{display:none!important;}',
      '.notes-preview{border:none!important;padding:0!important;}',
      '}'
    ].join('');
  ...
  function buildUI(root) {
    ...
    imagesTitle.textContent = '🖼 Прикріплені зображення';
    ...
    // Оновлення лічильника (спостерігаємо за масивом через renderThumbnails)
    var origRenderThumbnails = renderThumbnails;
    renderThumbnails = function () {
      imagesCount.textContent = state.imagesMeta.length + ' / ' + MAX_FILES;
      origRenderThumbnails();
    };
    ...
    dropZone.textContent = 'Перетягніть зображення сюди (JPG, PNG, WEBP — до 5 МБ кожне)';
  ...
  return {
    ...
    // Викликається APP_API.collectGlobalState()
    collectState: function () {
      // Текст беремо з textarea якщо є, інакше з state.text
      var currentText = elements.textarea ? elements.textarea.value : state.text;
      return {
        text: currentText,
        imagesMeta: state.imagesMeta.slice()
      };
    },

    // Викликається APP_API.loadData()
    restoreState: function (data) {
      if (!data) return;
      state.text = data.text || '';
      state.imagesMeta = Array.isArray(data.imagesMeta) ? data.imagesMeta : [];
      if (elements.textarea) {
        elements.textarea.value = state.text;
      }
      renderThumbnails();
    },

    // Чистить і текст і зображення (для "Очистити все")
    ...
    // Перевіряє чи є зображення (потрібно для рішення json/zip в APP_API)
    hasImages: function () {
      return state.imagesMeta.length > 0;
    },

    // Повертає масив {id, name, dataUrl} для пакування в ZIP
    ...
    getMarkdown: function () {
      ...
      if (text) md += text + '\n\n';
      if (state.imagesMeta.length > 0) {
        md += '### Прикріплені зображення\n\n';
        state.imagesMeta.forEach(function (m) {
          md += '- ' + m.name + ' (' + formatBytes(m.size) + ')\n';
        });
      }
      return md;
    }
