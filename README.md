...
сама назва файлу не впливає на URL, той визначається з `title`), заповніть:
...
(3 останні). Article schema (JSON-LD) генерується автоматично з `title`,
`description`, `date` — редагувати нічого додатково не потрібно.

Дати вказуйте у форматі `РРРР-ММ-ДД`. Список сортується за датою
(найновіші зверху).
