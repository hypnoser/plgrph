...
Калькулятор `/ps/` — **окремий проєкт** (репозиторій hypnoser/plgrph, той
самий, що продакшн plgrph.com/ps/), його код і логіку **не чіпаємо** при
роботі над рештою сайту. Він або підключається як окрема директорія в
`_site` при білді, або деплоїться незалежно — уточнити фінальний спосіб
інтеграції перед продакшн-релізом.
