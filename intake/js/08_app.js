...
el("case-save").addEventListener("click", async function(){
  ...
  /* Та сама логіка "не успадковувати від попереднього респондента"
     стосується й масштабу тексту: TEXT_ZOOM — глобальна змінна модуля,
     що переживає між справами в тій самій вкладці браузера. Новий
     респондент починає з дефолтних 100% незалежно від того, який
     масштаб було обрано для попереднього — увімкнене збільшення не
     повинно "перетікати" непомітно. */
  setTextZoom(100);
